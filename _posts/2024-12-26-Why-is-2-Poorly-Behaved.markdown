...
**Theorem C:** Let $K$ be a number field and $\text{cd}_p(K)$ denote the $p$-cohomological dimension of the absolute Galois group $G_K=\text{Gal}(\bar{K}/K)$. (I.e. For a profinite group $G$, $\text{cd}_p(G)$ is the smallest $n$ such that the $p$-part of $H^i(G,A)$ is trivial for any $i> n$ and any torsion $G$-module $A$.) Then $\text{cd}_p(G_K)=2$ unless $p=2$ and $K$ has a real place, in which case $\text{cd}_p(G_K)=\infty$. 
...
This reason is then also tied into the fact the $2$ is small but more for a meta reason. That being we study the small cases of problems before attacking the general case, so we're likely to study the $n=2$ case and therefore have $2$ in the set of bad primes.
...
So, what do these subgroups look like? Well for $\mathfrak{p}$ finite they behave very uniformly. We always have $\text{cd}\_p(G_{K_{\mathfrak{p}}})=2$ for all primes $p$. This is a really useful result and is deeply related to local class field theory. Now let $\mathfrak{p}\in S_\infty$. Here we have two cases. If $K_\mathfrak{p}=\mathbb{R}$, then $G_{K_{\mathfrak{p}}}=C_2$. If $K_{\mathfrak{p}}=\mathbb{C}$, then $G_{K_{\mathfrak{p}}}=1$. This second case is no problem: we have a bunch of trivial subgroups and they obey $\text{cd}\_p(G_{K_{\mathfrak{p}}})=0$. However, if $K_{\mathfrak{p}}=\mathbb{R}$, then we have a major issue: $\text{cd}\_2(G_{K_{\mathfrak{p}}})=\infty$.

I claim this is a unique reason that $2$ is poorly behaved: for a real prime $\mathfrak{p}$, the group $G_{\mathfrak{p}}\cong \text{Gal}(\mathbb{C}/\mathbb{R})\cong C_2$ has $\text{cd}\_2(C_2)=\infty$. Taking any $G\_{K_{\mathfrak{p}}}$ module $A$ with $H^i(G_{\mathfrak{p}},A)\neq 0$, we can use Shapiro's lemma to obtain a $G_K$ module $B=\text{ind}^{G_{\mathfrak{p}}}\_{G_K}A$ with $H^i(G_K,B)\cong H^i(G_{\mathfrak{p}},A)\neq 0$.
